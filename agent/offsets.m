...
bool resolvekernoffsets(NSString *kcpath) {
    // Guard: file harus ada dan tidak kosong
    if (!kcpath || ![[NSFileManager defaultManager] fileExistsAtPath:kcpath]) {
        printf("(offs) resolvekernoffsets: file tidak ada di %s\n", kcpath.UTF8String ?: "(null)");
        return false;
    }

    NSDictionary *attrs = [[NSFileManager defaultManager] attributesOfItemAtPath:kcpath error:nil];
    NSNumber *fileSize = attrs[NSFileSize];
    if (!fileSize || fileSize.longLongValue < 4096) {
        printf("(offs) resolvekernoffsets: file terlalu kecil (%lld bytes) — corrupt atau kosong\n",
    ...
    uint64_t allprocoff = allproc - gXPF.kernelBase;
    ...
    uint64_t tc_sym = ds_kcache_symbol_unslid("_trustcache");
    if (tc_sym) {
        printf("(kcache) symtab _trustcache unslid=0x%llx\n", tc_sym);
    }
...
bool dlkcache(void) {
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:outpath]) {
        printf("(offs) using cached kernelcache at %s\n", outpath.UTF8String);
        if (resolvekernoffsets(outpath)) {
            return true;
        }
        printf("(offs) cached kernelcache failed XPF — removing and re-downloading\n");
...
bool emergencyfixfunctiontobereplacedlateronquestionmark(void) {
    ...
    // Jangan crash jika kernelcache tidak ada — user belum import
    if (![[NSFileManager defaultManager] fileExistsAtPath:outpath]) {
        printf("(offs) kernelcache not found at %s — import dulu di Settings\n", outpath.UTF8String);
        return false;
    }
...
void offsets_init(void) {
    if (!(SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"17.0") && SYSTEM_VERSION_LESS_THAN(@"26.4"))) {
        printf("(offs) WARNING: offsets only verified for iOS 17.0 - 26.3\n");
        printf("(offs) Current iOS version may have incorrect offsets — proceed with caution\n");
        // Don't exit — let the exploit attempt and fail gracefully if offsets are wrong
    }
    ...
    if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"18.6")) {
        ...
        if(isA10) {
            off_thread_t_tro = 0x3B0;
            off_thread_ctid = 0x460;
            off_thread_mutex_lck_mtx_data = 0x3D8+8;
            off_thread_mach_exc_info_code = 0x360;
            off_thread_mach_exc_info_os_reason = 0x360-8;
            off_thread_mach_exc_info_exception_type = 0x360-4;
            off_thread_ast = 0x3D4;
            off_thread_task_threads_next = 0x3A0;
        }
    }
