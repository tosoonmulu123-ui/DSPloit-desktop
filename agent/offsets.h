...
/// Sentinel value indicating an offset is not available on the current iOS version.
/// Code MUST check `off_xxx != OFFSET_INVALID` before using as a kernel address.
#define OFFSET_INVALID 0xdeaddead

/// Helper macro: returns true if an offset is valid (not the sentinel value)
#define OFFSET_IS_VALID(off) ((off) != OFFSET_INVALID)
